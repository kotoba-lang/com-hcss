(ns hcss.main
  "Kotodama WASM entrypoint for the Hcss clean-room actor (L4) — Clojure port.
  ...
  facts (`hcss.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"hcss.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "hcss-compat" :tier tier :entities entities} 200])
